...
<head>
    ...
    <script type="text/javascript">
        ...
        function searchTeacher(value,name){
            searchType=name;
            searchValue=value;
            realListTeachers('${lesson}/details/searchTeacherById');
        }

        function listTeachers(){
            realListTeachers('${lesson}/details/listTeachers')
        }

        //显示教师函数
        function realListTeachers(toUrl){
            //datagrid初始化
            $('#dg').datagrid({
                //发送的额外参数
                queryParams: {
                ...
                toolbar:[{
                    ...
                    handler: function () {  //回调函数
                        ...
                        //添加url
                        url = "${lesson}/admin/insertTeacher"
                    }
                },'-',{
                    iconCls: 'icon-edit',
                    text: '修改',
                    handler: function () {
                        ...
                        if(selectedRows.length != 1) {
                            $.messager.alert("系统提示", "请选择一个要修改的教师");
                            return;
                        }
                        //获取选中行id
                        ...
                        $("#oldId").val(row.id);
                        //url修改
                        url = "${lesson}/admin/updateTeacher" ;
                    }
                },'-',{
                    iconCls: 'icon-remove',
                    text: '删除',
                    handler: function () {
                        ...
                        $.messager.confirm("系统提示", "<font color=red>您确定要删除选中的"+selectedRows.length+"条数据么？</font>", function(r) {
                            if(r) {
                                $.post("${lesson}/admin/deleteTeacher",
                                    {ids: ids}, function(result){
                                        if(result.exist) {
                                            $.messager.alert("系统提示", '教师下有班级或课程不能删除');
    ...
    <tr>
        <td>
            <input id="ss" class="easyui-searchbox" style="width:260px"
                   data-options="searcher:searchTeacher,prompt:'Please Input Value',menu:'#mm'"></input>
        ...
        <td align="right">
            <a href="#" onclick="listTeachers()" class="easyui-linkbutton" iconCls="icon-search">显示所有教师</a>
            <%--<input type="button" onclick="listTeachers()" style="text-align: right"  value="显示所有教师">--%>
        </td>
    </tr>
</table>
...
<div id="dlg-buttons">
    <div>
        <a href="javascript:saveTeacher()" class="easyui-linkbutton" iconCls="icon-ok" plain="true">保存</a>
